...
import { CmsMessage, LooseObject, SocketNextFunction, CrudRoutes } from "@cmsTypes/index";
...
export class MessageParser extends IncomingParser {
    public middleware(packet: Event, next: SocketNextFunction): void {
        ...
        try {
            packet[1] = { ...packet[1], ...this.parseMessage(msg, eventName as keyof CrudRoutes) } as CmsMessage;
        } catch (error) {
            return next(new SocketError(String(error), msg.id));
        }
        next();
    }
    public parseMessage(msg: LooseObject, crudMethodName: keyof CrudRoutes): CmsMessage {
